...
#job.name=Repjoin.TwitterTriangleCountMain
#job.name=approx_2_hop.TwitterFollowerApproxCount
#job.name=exact_2_hop.TwitterFollowerCount
job.name=RSjoin.TwitterTriangleCountMain
#job.name=Repjoin.TwitterFollowerTriangleMain
local.input=input
local.output=output
...
aws.bucket.name=cs6240-hw2-bucket-pritish
aws.subnet.id=subnet-6356553a
aws.input=input
aws.output=output
aws.log.dir=log
aws.primary.num.nodes=1
aws.core.num.nodes=3
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		--name "TwitterData MR Cluster RS Join Mark 2 (MAX 30k - 3w)" \
...
download-log-aws:
	mkdir mr_logs_aws
	aws s3 sync s3://${aws.bucket.name}/${aws.log.dir} mr_logs_aws
